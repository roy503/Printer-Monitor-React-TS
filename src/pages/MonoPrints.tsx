...
// MonoPrints component
export function MonoPrints() {
  ...
  // potentially include last year december printers? to show january monthly
  const currentYearPrinters = (printerList?.[selectedYear] ?? []).filter(
    (printer) => printer.monoPrints > 0
  );
  ...
  return (
    <>
      {/* Table displaying mono prints information */}
            ...
            ? currentYearPrinters
                .sort((a, b) => {
                  ...
                  if (
                    ...
                    // Access the MonoPrints value for the selected month
                    const monthA =
                      indexA !== undefined
                        ? (a.Reports[indexA]?.monoPrints || 0) -
                          (a.Reports[indexA - 1]?.monoPrints || 0)
                        : 0;
                    const monthB =
                      indexB !== undefined
                        ? (b.Reports[indexB]?.monoPrints || 0) -
                          (b.Reports[indexB - 1]?.monoPrints || 0)
                        : 0;
                    // Compare the MonoPrints values
                ...
                .map((printer) => {
                  ...
                  return (
                    <tr key={printer.address}>
                      ...
                      {/* Map over reports and display mono prints for each month */}
                      {printer.Reports?.map((report: Reports) => {
                        let currentMonthMonoPrints =
                          report.monoPrints - prevMonthMonoPrints;
                        if (
                          prevMonthMonoPrints == -1 ||
                          prevMonthMonoPrints == 0
                        ) {
                          currentMonthMonoPrints = -1;
                          if (report.monoPrints > 0) {
                            currentMonthMonoPrints = 0;
                          }
                        }
                        prevMonthMonoPrints = report.monoPrints;
                        if (prevMonthMonoPrints < 0) {
                          prevMonthMonoPrints = 0;
                        }
                        return (
                          <td key={report.month}>
                            {/* Display mono prints if available, otherwise display an empty string */}
                            {currentMonthMonoPrints > -1
                              ? currentMonthMonoPrints
                              : ""}
                          </td>
                        );
                      })}
